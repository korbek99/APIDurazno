...
class Reclamo extends REST_Controller
{
    ...
    public function shares_html_get()
    {
                ...
                $resultresponse = "";
                $domain_folder_destination = "assets/";
                $foto_url2 = "";

                $imagen_antes = $this->get('imagen_antes');
                $imagen_despues = $this->get('imagen_despues');
                $imagen_tercera = $this->get('imagen_tercera');

               //$imagen_antes = "http://www.jqueryscript.net/images/Simplest-Responsive-jQuery-Image-Lightbox-Plugin-simple-lightbox.jpg";
               //$imagen_despues = "https://upload.wikimedia.org/wikipedia/commons/0/0f/Eiffel_Tower_Vertical.JPG";

              // $imagen_tercera = "https://s3.amazonaws.com/iddurazno/logo/logo_fbshare.png";
               //$result['codigo']   = '1';
               //$result['mensaje']  = 'imagen para compartir en redes sociales';

                //$result['data']  = $this->getImageFromReclamo($imagen_antes,$imagen_despues);
                
               // $imagen_facebook = $result['data'];
               // $imagen_twitter = "";
                ...
                $page = str_replace('{imagefacebook}', $imagen_tercera, $page);
                //echo $page;
                $unique_file_names = md5(uniqid(mt_rand())).".html";
                $nombre_archivo = "assets/uploads/files/".$unique_file_names;
                ...
                $resultresponse = $mensaje;
                //$unique_file_name = md5(uniqid(mt_rand())).".html";
                $file_path = $nombre_archivo; // "assets/uploads/files/".$unique_file_names;
                //imagepng($dest,$file_path);
                // echo $file_path;
                $domain_folder_destination = 'share_reclamo';
                $image =  $this->uploadhelper->uploadImagePathAndGetUrlNews($domain_folder_destination,$file_path);
                //echo $image;
                //imagedestroy($dest);
                //imagedestroy($src);

                $resultres['codigo']   = '1';
                $resultres['mensaje']  = 'pagina para compartir en redes sociales';
                $resultres['data']  = $image;
                $this->set_response($resultres, REST_Controller::HTTP_OK);
